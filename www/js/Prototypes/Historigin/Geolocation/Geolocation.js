...
Geolocation.prototype.getCollection = function(lat, lng, callback) {
	$.get(TYPEF_HOST + "/app/refresh?lat=" + encodeURIComponent(lat) + '&lng=' + encodeURIComponent(lng) + '&radius=' + encodeURIComponent(this.radius), function(data) {
		//console.log(data);
		if (!empty(data)) {
...
Geolocation.prototype.getCollectionByScope = function(lat1, lng1, lat2, lng2, centerlat, centerlng, callback) {
